...
public class MP2006 {
	...
	public void test() throws Exception {
		...
		driverWait.until(ExpectedConditions.presenceOfElementLocated(
				By.xpath("//aside[@id='left-panel']/navigation-widget/div/nav/ul/li[7]/a[2]/span"))).click();

		driverWait.until(ExpectedConditions.presenceOfElementLocated(
				By.linkText("Confirm SRO Notification of Claim Registration - External Client"))).click();
	...
	private void updateIssueDate() throws ClassNotFoundException, SQLException {
		Connection con = DbConnection.getConnection();
		con.setAutoCommit(false);
		Statement stmt = con.createStatement();
		String sql = ("UPDATE MAM.MTA_TENURE SET ISSUE_DATE = ISSUE_DATE  - 61  WHERE TENURE_NUMBER_ID = " + claimId);
		stmt.executeUpdate(sql);
		con.commit();
		con.close();
